...
Pages.industry = {
  ...
  _renderDualTabs(type, ind, owned) {
    ...
    h += '<div class="tab-bar">';
    h += '<div class="tab' + (activeTab === 'owned' ? ' active' : '') + '" onclick="Pages.industry.switchTab(\'' + type + '\', \'owned\', event)">我的产业 (' + ownedCategories.length + ')</div>';
    h += '<div class="tab' + (activeTab === 'shop' ? ' active' : '') + '" onclick="Pages.industry.switchTab(\'' + type + '\', \'shop\', event)">可购入 (' + unownedCategories.length + ')</div>';
    h += '</div>';
  ...
  _renderLogisticsPanel(owned) {
    ...
    if (rules.length === 0) {
      ...
      for (var ri = 0; ri < rules.length; ri++) {
        ...
        h += '<button class="btn sm" style="min-height:28px;font-size:11px;" onclick="LogisticsSystem.removeRule(' + ri + ');Pages.industry._refreshLogisticsPanel();UI.toast(\'\u5df2\u5220\u9664\u89c4\u5219\')">\u5220\u9664</button></div>';
    ...
    // add button
    h += '<button class="btn full primary" style="margin-top:8px;" ' + (freeSlots <= 0 ? 'disabled style="opacity:0.4;"' : '') + ' onclick="Pages.industry._showAddRuleModal()">';
  ...
  _showAddRuleModal() {
    ...
    for (var i = 0; i < materials.length; i++) {
      ...
      matRows += '<div class="mat-item" data-code="' + m.code + '" onclick="Pages.industry._selectMaterial(\'' + id + '\', \'' + m.code + '\')">';
    ...
    content += '<input type="text" id="' + id + '_search" placeholder="\u8f93\u5165\u7269\u6599\u540d\u79f0\u641c\u7d22..." oninput="Pages.industry._filterMaterials(\'' + id + '\')" style="width:100%;padding:10px;border-radius:var(--radius-md);border:0.5px solid var(--border-strong);background:var(--bg-card);color:var(--text-primary);font-size:14px;margin-bottom:6px;">';
    ...
    UI.modal('\u6dfb\u52a0\u7269\u6d41\u89c4\u5219', content, [
      ...
      { label: '\u786e\u8ba4\u6dfb\u52a0', class: 'primary', onclick: 'Pages.industry._confirmAddRule("' + id + '")' }
    ]);
  },
  ...
  purchaseItem(type, cat) {
    ...
    const isCap = Pages.industry._isCapacity(type) && !cat.cost;
    const capTotal = isCap ? Pages.industry._totalCapacity(type) : 0;
    const capUsed = isCap ? Pages.industry._usedCapacity(type) : 0;
...
const Industry = {
  ...
  buy(type, categoryCode) {
    ...
    if (Pages.industry._isCapacity(type) && !cat.cost) {
...
Industry._allocSubmit = function() {
  ...
  UI.toast('产品分配已更新');
  Pages.industry._refreshCapacity(st.type);
};
